...
# 页面标题用字体路径（assets/AlibabaPuHuiTi-3-105-Heavy.ttf）
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
HEADER_FONT = os.path.join(BASE_DIR, 'assets', 'AlibabaPuHuiTi-3-105-Heavy.ttf')
...
def draw_page_frame(disp, title, title_color=BLACK, font_path=None):
    ...
    标题默认使用 HEADER_FONT（assets/AlibabaPuHuiTi-3-105-Heavy.ttf），
    可传入自定义 font_path 覆盖。
    """
    W = disp.width
    H = disp.height
